...
// const foodlist = ref([]);
// foodlist.value = await fetch(`${baseUrl}`)
// .then(response => response.json())
// .then(data => data.categories.slice(0,6));
// console.log(foodlist)

// let newfoodList = foodlist.slice(0,7)
// console.log(newfoodList)
const foodlist = ref({});
fetch(`${baseUrl}`)
.then(response => response.json())
.then(data => {
    foodlist.value = data.categories.slice(0,6)
});

</script>
<template>
    <section class="grid grid-cols-2 lg:grid-cols-1  gap-2 sm:gap-4">
